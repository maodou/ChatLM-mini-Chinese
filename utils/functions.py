...
class MyTrainerCallback(TrainerCallback):
    ...
    def on_log(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        self.log_cnt += 1
        if self.log_cnt % 2 == 0:
            print("memory used before empty cache: %.2fG  \n" % self.get_memory_used())
            print("start empty cache ..... \n")
            torch.cuda.empty_cache()
            # torch.mps.empty_cache()

            print("end empty cache ..... \n")
            print("memory used after empty cache: %.2fG \n" % self.get_memory_used())
...
def get_T5_config(config: T5ModelConfig, vocab_size: int, decoder_start_token_id: int = 0,
